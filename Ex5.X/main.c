...
void main(void) {
  ...
  char lcd[5];         //string para armazenar os valores coletados
  int i;               //contador
  int i_max = 0;       //valor maximo do contador
  ...
  while(1) {
      if(rch == '*'){
          ...
          for(i = 0; i <= i_max; i++){
            if(i == 0){
                //tenta armazenar o valor do botão em lcd[i] caso a string seja vazia
                //atualiza porta de acordo com o contador
                PORTA = 0b10000000;
                if(rch != '\0')
                    lcd[i] = rch;
            }
